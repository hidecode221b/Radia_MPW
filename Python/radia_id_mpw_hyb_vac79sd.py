...
class HybridWigParam(UndParam):
    ...
    """
    def __init__(self, period, n_poles, gap, mag_width=79, mag_height=[79, 79, 79], mag_chamfer=[10, 0, 10], pole_length=15, pole_width=79, pole_height=61, pole_chamfer=[10, 0, 10], ext_pole=[9, 0], ext_mag=[79, 0, 10], mag_mat='ndfeb', br=1.29, sep_exp=[0, 0, 0], pole_mat='fecov', mag_area_max=200, pole_area_max=100, mag_long_sub=[2, 2], pole_long_sub=[1, 1], mag_color=[[0, 1, 1],[0, 0.5, 1]], msm=[0, 1, 0.001], mss=[1, 0, 0], pole_color=[1, 0, 1], wig_build='full'):
        """
        ...
        if n_poles % 2 == 0 and n_poles > 0:
            if mag_id == 1:
                if half_period_id % 2 == 0:
                    axis = [msm_x, msm_y, msm_z]
                else:
                    axis = [msm_x, -1*msm_y, msm_z]
            else:
                if half_period_id % 2 == 0:
                    axis = [msm_x, -1*msm_y, msm_z]
                else:
